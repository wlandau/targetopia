...
In `_targets.R`, the user writes one call to the factory instead of multiple calls to `tar_target()`. This shorthand simplifies pipeline construction by reducing the volume of code required and abstracting away settings like `format = "file"`.
...
target_factory <- function(name, data) {
  ...
  list(
    tar_target_raw(name_file, data, format = "file"),
    tar_target_raw(name_text, command)
  )
}
```
